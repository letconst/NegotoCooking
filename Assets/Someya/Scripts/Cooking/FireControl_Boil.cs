...
public class FireControl_Boil : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            if (Otama.transform.position.z <= -6)
            {
                Otama.transform.position = new Vector3(Otama.transform.position.x, Otama.transform.position.y, Otama.transform.position.z);
            }

            if (Otama.transform.position.z > -6)
            {
                Otama.transform.position = new Vector3(Otama.transform.position.x, Otama.transform.position.y, Otama.transform.position.z - 1.5f);
            }
        }

        if (Stick_V < 0 || Input.GetKeyDown(KeyCode.W))
        {
            if (Otama.transform.position.z >= 114)
            {
                Debug.Log("114ue");
                Otama.transform.position = new Vector3(Otama.transform.position.x, Otama.transform.position.y, Otama.transform.position.z);
            }
